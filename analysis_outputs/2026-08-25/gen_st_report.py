...
# 实时数据（腾讯 qt.gtimg.cn 2026-08-20 收盘，市值单位为亿元；cap_anom=True 表示市值字段异常待复核）
data = [
 # name, code, price, chg%, turnover%, total_cap(亿), cap_anom, status_note,
 # scores(fund,liq,restruct,turn,risk 1-5), catalyst, risk
 ("ST宁科","600165",3.23,5.90,3.36,40.87,False,"已向上交所申请撤销其他风险警示(8/14)",
   (5,4,5,4,3.5),
 ...
 ("ST任子行","300311",6.11,-2.24,6.19,58.80,False,"行情显示名已无ST前缀，或已摘帽/处于审核末期(以最新公告为准)",
   (5,4,5,4,4),
   "①网络安全+数据中心赛道，摘帽进度市场普遍认为最靠前；②底部企稳、主力控盘特征明显；③一旦摘帽估值修复+情绪双击。",
   "①摘帽审批权在交易所，节奏不确定；②预期或已部分透支，落地后谨防回落。"),
...
for (name,code,price,chg,turn,cap,anom,status,scores,cat,risk) in data:
    ...
    rows.append(dict(name=name,code=code,price=price,chg=chg,turn=turn,cap=cap,anom=anom,
                     status=status,fund=fund,liq=liq,restruct=restruct,turn_s=turn_s,risk=risk_s,
                     total=round(total,3),cat=cat,risk_txt=risk))
rows.sort(key=lambda r:-r["total"])

# 已完成反转(已摘帽/更名)参照标的
done = [
 ("松发股份","603268",176.21,"恒力重工100%股权置入，2025/5交割、2026/3申请摘帽"),
 ("石药景峰","000908",7.00,"石药控股入主，重整完成，转型创新药平台"),
 ("东易日盛","002713",9.45,"重整完成，北京华著科技入主，注入算力订单转型AI家装+算力"),
 ("宇顺电子","002289",41.50,"现金收购数据中心资产，转型算力"),
 ("国旅联合","600358",5.07,"润田实业100%股权注入，恢复审核"),
 ("顺景科技","603007",4.59,"原ST花王重整完成，转型新能源/半导体/新基建"),
]
...
<title>ST/*ST 反转预期优先级清单 (2026-08-20)</title>
<style>
:root{{--bg:#0f1419;--card:#1a2330;--line:#2a3645;--txt:#e6edf3;--mut:#9fb0c0;--acc:#4ea1ff;--up:#ff5b5b;--down:#26c281;--gold:#ffcf5c}}
*{{box-sizing:border-box}}
body{{margin:0;background:var(--bg);color:var(--txt);font-family:-apple-system,'Segoe UI','Microsoft YaHei',sans-serif;line-height:1.6}}
...
th{{background:#16202c;color:var(--mut);font-weight:600}}
td.l,th.l{{text-align:left}}
.rank{{color:var(--gold);font-weight:700;font-size:15px}}
...
.code{{color:var(--mut);font-size:12px;font-weight:400;margin-left:8px}}
.scores{{display:flex;flex-wrap:wrap;gap:6px;margin:8px 0}}
.sc{{background:#0e1620;border:1px solid var(--line);border-radius:6px;padding:3px 9px;font-size:11.5px;color:var(--mut)}}
.sc b{{color:var(--txt)}}
.cat,.rk{{margin:6px 0;font-size:13.5px}}
.cat b{{color:var(--gold)}}.rk b{{color:#ff9b9b}}
.anom{{color:#ffcf5c;font-size:11.5px}}
.tag{{display:inline-block;background:#10212e;border:1px solid var(--acc);color:var(--acc);border-radius:5px;padding:1px 7px;font-size:11px;margin-left:6px}}
.foot{{color:var(--mut);font-size:12px;margin-top:30px;border-top:1px solid var(--line);padding-top:14px}}
</style></head><body><div class="wrap">
<h1>ST / *ST 反转预期 · 投资价值优先级清单</h1>
<div class="sub">数据基准：2026-08-20 收盘（腾讯实时行情核验）｜评分模型：五维加权｜生成时间：{datetime.date.today().isoformat()}</div>
...
<tr><th>优先级</th><th class="l">标的</th><th>现价</th><th>涨跌幅</th><th>换手%</th><th>总市值</th><th>基本面</th><th>市值流动</th><th>重组进度</th><th>扭亏概率</th><th>风险可控</th><th>综合</th></tr>""")
for i,r in enumerate(rows,1):
    chg_cls = "up" if r["chg"]>=0 else "down"
    sign = "+" if r["chg"]>=0 else ""
    cap_disp = f'{r["cap"]}亿<span class="anom">*</span>' if r["anom"] else f'{r["cap"]}亿'
    html_parts.append(f"""<tr>
<td class="rank">{i}</td>
<td class="l"><b>{r['name']}</b><span class="code">{r['code']}</span></td>
<td>{r['price']}</td>
<td class="{chg_cls}">{sign}{r['chg']}%</td>
<td>{r['turn']}</td>
<td>{cap_disp}</td>
<td>{r['fund']}</td><td>{r['liq']}</td><td>{r['restruct']}</td><td>{r['turn_s']}</td><td>{r['risk']}</td>
<td class="rank">{r['total']}</td></tr>""")
html_parts.append("</table>")
html_parts.append('<div class="anom">* 该标的市值字段在行情源显示异常（流通&gt;总或显著偏离板块常态），已标注待复核，评分中流动性维度按审慎处理。</div>')

# 分级
def tier_block(title, items):
    out=[f'<div class="tier">{title}</div>']
    for r in items:
        chg_cls="up" if r["chg"]>=0 else "down"
        sign="+" if r["chg"]>=0 else ""
        cap_disp=f'{r["cap"]}亿*' if r["anom"] else f'{r["cap"]}亿'
        out.append(f"""<div class="card">
<h3>{r['name']}<span class="code">{r['code']}｜现价 {r['price']}（{sign}{r['chg']}%）｜总市值 {cap_disp}｜换手 {r['turn']}%</span></h3>
<div class="scores">
<span class="sc">基本面 <b>{r['fund']}</b></span><span class="sc">市值流动 <b>{r['liq']}</b></span>
<span class="sc">重组进度 <b>{r['restruct']}</b></span><span class="sc">扭亏概率 <b>{r['turn_s']}</b></span>
<span class="sc">风险可控 <b>{r['risk']}</b></span><span class="sc">综合 <b>{r['total']}</b></span></div>
<div style="color:var(--mut);font-size:12px;margin:2px 0 8px">状态：{r['status']}</div>
<div class="cat"><b>核心催化逻辑：</b>{r['cat']}</div>
<div class="rk"><b>主要风险：</b>{r['risk_txt']}</div>
</div>""")
    return "\n".join(out)
...
html_parts.append(tier_block("【P1 高确定性 · 近期催化（摘帽申请/重整执行完毕）】", p1))
html_parts.append(tier_block("【P2 中高确定性 · 基本面反转/扭亏兑现】", p2))
html_parts.append(tier_block("【P3 中报扭亏 · 主业改善，待摘帽条件验证】", p3))
html_parts.append(tier_block("【P4 进展中 · 需持续跟踪】", p4))

# 已完成反转参照
html_parts.append('<div class="tier">【已摘帽/更名 · 反转已兑现的参照标的】</div>')
html_parts.append('<table><tr><th class="l">标的</th><th>现价</th><th class="l">反转路径</th></tr>')
for n,c,p,desc in done:
    html_parts.append(f'<tr><td class="l"><b>{n}</b><span class="code">{c}</span></td><td>{p}</td><td class="l">{desc}</td></tr>')
html_parts.append('</table>')

html_parts.append(f"""<div class="foot">
<b>方法论：</b>五维加权评分（各维度1-5分）：基本面改善确定性(权重25%)＋市值与流动性(15%)＋重组进度(25%)＋业绩预告扭亏概率(20%)＋风险可控性(15%)，综合分=加权求和。<br>
<b>数据来源：</b>行情与市值来自腾讯 qt.gtimg.cn（2026-08-20 收盘）；催化与风险依据证券时报·数据宝、东方财富、同花顺、雪球公开公告梳理（截至2026-08-20）。<br>
<b>重要提示：</b>ST/*ST 板块波动剧烈，*ST 类仍处退市风险警示；本清单中市值异常项、摘帽审核结果、重组审批均存在不确定性，请以交易所与上市公司最新公告为准。
</div></div></body></html>""")

out = "\n".join(html_parts)
with open(r"C:\Users\EDY\AppData\Local\Temp\wb_analysis\ST_reversal_priority_20260820.html","w",encoding="utf-8") as f:
    f.write(out)
# 同时保存评分明细 json
with open(r"C:\Users\EDY\AppData\Local\Temp\wb_analysis\ST_reversal_scores_20260820.json","w",encoding="utf-8") as f:
    json.dump(rows, f, ensure_ascii=False, indent=2)
print("OK 已生成 HTML + 评分JSON")
print("排名:", " > ".join(f"{i+1}.{r['name']}({r['total']})" for i,r in enumerate(rows)))
